...
def video_capture(frame_queue ,width,height):
    while cap.isOpened():
        ret, frame = cap.read()
        if ret:
            ###
...
##### GRAPES
    ...
    cap = cv2.VideoCapture("/home/berno/Documents/Image_labeling/1_2_R.MP4")

    Thread(target=video_capture, args=(frame_queue,width,height)).start()

    time.sleep(1.0)

    target_res_ratio = (576/height)
